...
ll query(vl& segTree, ll a, ll b, ll l, ll r, ll i){
    if(b<l || a>r) return 0;
    if(l>=a && r<=b) return segTree[i];
    ll mid = l + (r-l)/2;
    ll x = query(segTree, a, b, l, mid, i*2+1);
    ll y = query(segTree, a, b, mid+1, r, i*2+2);
    return __gcd(x,y);
}
...
void solve(){
    ...
    while(q--){
        ...
        cin >> a >> b;
        a--; b--;
        ll x = query(segTree, a, b, 0, n-1, 0);
        // cout(x);
        auto lb = lower_bound(mp[x].begin(), mp[x].end(), a);
        auto ub = upper_bound(mp[x].begin(), mp[x].end(), b);
        ll ans = distance(lb, ub);
        cout(b-a+1-ans);
    }
}
int main(){
